...
export function ResolutionImprovement() {
  const t = useScopedI18n('resolution-improvement.tool');
  ...
  const {
    trigger: resolutionImprovement,
    isMutating: isProcessing,
    error,
    reset,
    data: generatedImage,
  } = useSWRMutation('/api/resolution-improvement', async (url, { arg }: { arg: FormData }) => {
  ...
  const submit = form.handleSubmit(async (data: z.infer<typeof imageEditSchema>) => {
    try {
      ...
      await resolutionImprovement(formData);
    } catch (err) {
