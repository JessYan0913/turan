...
export function MyWorksPage({ isDarkMode, themeClasses }: MyWorksPageProps) {
      ...
      {filteredWorks.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredWorks.map((work) => (
            <Card
              key={work.id}
              className={`${themeClasses.card} border-0 shadow-lg hover:shadow-xl transition-all duration-300 group`}
            >
                ...
                <div className="relative h-48 overflow-hidden rounded-t-lg">
                  <div className="grid grid-cols-2 h-full">
                    <div className="relative">
                      <Image src={work.originalImage || "/placeholder.svg"} alt="原图" fill className="object-cover" />
